//
//  ADOPMediationUnifiedNativeAdView.swift
//  MediationADOP
//
...
/// ADOP 통합형 네이티브 광고의 실제 로드/바인딩 담당.
/// GADAdLoader로 광고를 로드하고, ViewBinder에 데이터를 바인딩합니다.
final class ADOPMediationUnifiedNativeAdView: UIView {
    ...
    deinit {
        APLogger.debug("ADOPMediationUnifiedNativeAdView deinit")
    }
    ...
    func load() {
        guard !placementId.isEmpty else {
            APLogger.error("ADOP UnifiedNative placementId is empty")
...
private extension ADOPMediationUnifiedNativeAdView {
...
private extension ADOPMediationUnifiedNativeAdView {
    ...
    func handleNativeAdWithContentView(_ nativeAd: NativeAd) {
        guard let placeholder = viewBinder.resolvedPlaceholder else {
            delegate?.unifiedNativeLoadFail(error: .nextMediation, errorMessage: "ADOP placeholder is nil")
            return
        }

        guard let content = viewBinder.makeContentView() else {
            delegate?.unifiedNativeLoadFail(error: .nextMediation, errorMessage: "ADOP contentView 생성 실패")
            return
        }
        ...
        if APSSPUnifiedNativeAssembler.fillSlot(content.mediaContainerView, with: mediaView) {
            ...
            APLogger.error("ADOP UnifiedNative: mediaContainerView가 없습니다. XIB에 빈 컨테이너를 배치하세요.")
        }
...
// MARK: - GAD Delegates
extension ADOPMediationUnifiedNativeAdView: NativeAdLoaderDelegate, NativeAdDelegate {

    public func adLoader(_ adLoader: AdLoader, didReceive nativeAd: NativeAd) {
        self.nativeAd = nativeAd
        if viewBinder.isContentViewMode {
            APLogger.debug("ADOP UnifiedNative → 매체 XIB (신규 구조)")
    ...
    public func adLoader(_ adLoader: AdLoader, didFailToReceiveAdWithError error: Error) {
        APLogger.error("ADOP UnifiedNative Error: \(error.localizedDescription)")
    ...
    public func nativeAdDidRecordClick(_ nativeAd: NativeAd) {
        delegate?.unifiedNativeClicked(message: "ADOP UnifiedNative clicked")
    }
    
    public func nativeAdDidRecordImpression(_ nativeAd: NativeAd) {
        delegate?.unifiedNativeImpression(message: "ADOP UnifiedNative impression")
    }
}
