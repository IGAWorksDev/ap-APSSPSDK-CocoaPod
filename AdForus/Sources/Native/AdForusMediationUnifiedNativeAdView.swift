//
//  AdForusMediationUnifiedNativeAdView.swift
//  MediationAdForus
//
...
/// AdForus 통합형 네이티브 광고의 실제 로드/바인딩 담당.
/// GADAdLoader로 광고를 로드하고, ViewBinder에 데이터를 바인딩합니다.
final class AdForusMediationUnifiedNativeAdView: UIView {
    ...
    deinit {
        APLogger.debug("AdForusMediationUnifiedNativeAdView deinit")
    }
    ...
    func load() {
        guard !placementId.isEmpty else {
            APLogger.error("AdForus UnifiedNative placementId is empty")
...
private extension AdForusMediationUnifiedNativeAdView {
...
private extension AdForusMediationUnifiedNativeAdView {
    ...
    func handleNativeAdWithContentView(_ nativeAd: NativeAd) {
        guard let placeholder = viewBinder.resolvedPlaceholder else {
            delegate?.unifiedNativeLoadFail(error: .nextMediation, errorMessage: "AdForus placeholder is nil")
            return
        }

        guard let content = viewBinder.makeContentView() else {
            delegate?.unifiedNativeLoadFail(error: .nextMediation, errorMessage: "AdForus contentView 생성 실패")
            return
        }
        ...
        if APSSPUnifiedNativeAssembler.fillSlot(content.mediaContainerView, with: mediaView) {
            ...
            APLogger.error("AdForus UnifiedNative: mediaContainerView가 없습니다. XIB에 빈 컨테이너를 배치하세요.")
        }
...
// MARK: - GAD Delegates
extension AdForusMediationUnifiedNativeAdView: NativeAdLoaderDelegate, NativeAdDelegate {

    public func adLoader(_ adLoader: AdLoader, didReceive nativeAd: NativeAd) {
        self.nativeAd = nativeAd
        if viewBinder.isContentViewMode {
            APLogger.debug("AdForus UnifiedNative → 매체 XIB (신규 구조)")
    ...
    public func adLoader(_ adLoader: AdLoader, didFailToReceiveAdWithError error: Error) {
        APLogger.error("AdForus UnifiedNative Error: \(error.localizedDescription)")
    ...
    public func nativeAdDidRecordClick(_ nativeAd: NativeAd) {
        delegate?.unifiedNativeClicked(message: "AdForus UnifiedNative clicked")
    }
    
    public func nativeAdDidRecordImpression(_ nativeAd: NativeAd) {
        delegate?.unifiedNativeImpression(message: "AdForus UnifiedNative impression")
    }
}
