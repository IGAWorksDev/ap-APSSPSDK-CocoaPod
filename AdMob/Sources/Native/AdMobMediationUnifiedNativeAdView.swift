...
private extension AdMobMediationUnifiedNativeAdView {
    ...
    func handleNativeAdWithContentView(_ nativeAd: NativeAd) {
        guard let placeholder = viewBinder.resolvedPlaceholder else {
            delegate?.unifiedNativeLoadFail(error: .nextMediation, errorMessage: "AdMob placeholder is nil")
            return
        }

        guard let content = viewBinder.makeContentView() else {
            delegate?.unifiedNativeLoadFail(error: .nextMediation, errorMessage: "AdMob contentView 생성 실패")
            return
        }
        ...
        if APSSPUnifiedNativeAssembler.fillSlot(content.mediaContainerView, with: mediaView) {
            ...
            APLogger.error("AdMob UnifiedNative: mediaContainerView가 없습니다. XIB에 빈 컨테이너를 배치하세요.")
        }
...
extension AdMobMediationUnifiedNativeAdView: NativeAdLoaderDelegate, NativeAdDelegate {
    ...
    public func adLoader(_ adLoader: AdLoader, didReceive nativeAd: NativeAd) {
        ...
        if viewBinder.isContentViewMode {
            APLogger.debug("AdMob UnifiedNative → 매체 XIB (신규 구조)")
